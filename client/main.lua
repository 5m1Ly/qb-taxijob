...
local function calculateFareAmount()
    if meterIsOpen and meterActive then
        ...
        if startPos ~= newPos then
            ...
            local fareAmount = (meterData['distanceTraveled'] / 100.00) * meterData['fareAmount']
    ...
    if whitelistedVehicle() then
        if not NpcData.Active then
           ...
           -- added checks to disable distance checking if polyzone option is used
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    elseif Config.NotifyType == "okok" then
                                        exports['okokNotify']:Alert("RIJD NAAR LOCATIE", Lang:t("info.go_to_location"), 3500, "info")
                                    end
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
            ...
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("JE BENT AL BEZIG", Lang:t("error.already_mission"), 3500, "error")
            end
        end
    else
        if Config.NotifyType == 'qb' then
        ...
        elseif Config.NotifyType == "okok" then
            exports['okokNotify']:Alert("NIET IN TAXI", Lang:t("error.not_in_taxi"), 3500, "error")
        end
    end
end)
...
RegisterNetEvent('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            elseif Config.NotifyType == "okok" then
                exports['okokNotify']:Alert("GEEN METER", Lang:t("error.missing_meter"), 3500, "error")
            end
        end
    else
        if Config.NotifyType == 'qb' then
            ...
            if isInsidePickupZone then
                if IsControlJustPressed(0, 38) then
                    ...
                    if Config.NotifyType == 'qb' then
                        QBCore.Functions.Notify(Lang:t("info.go_to_location"))
                    elseif Config.NotifyType == "okok" then
                        exports['okokNotify']:Alert("RIJDT KLANT", Lang:t("info.go_to_location"), 3500, "success")
                    end
                    if NpcData.NpcBlip ~= nil then
                        RemoveBlip(NpcData.NpcBlip)
                    end
            ...
            if isInsideDropZone then
                if IsControlJustPressed(0, 38) then
                    ...
                    if Config.NotifyType == 'qb' then
                        QBCore.Functions.Notify(Lang:t("info.person_was_dropped_off"), 'success')
                    elseif Config.NotifyType == "okok" then
                        exports['okokNotify']:Alert("KLANT AFGEZET", Lang:t("info.person_was_dropped_off"), 3500, "success")
                    end
                    if NpcData.DeliveryBlip ~= nil then
                        RemoveBlip(NpcData.DeliveryBlip)
                    end
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    elseif Config.NotifyType == "okok" then
                        exports['okokNotify']:Alert("TAXI TERUGGEBRACHT", Lang:t("info.taxi_returned"), 3500, "info")
                    end
                end
            end
        end
